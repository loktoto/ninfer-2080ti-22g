...
// ninfer::ops - signed int8 and packed 4-bit, per-token group-wise KV cache codec helpers.
// Quantization (append) and dequantization (stage) remain fused into the GQA attention kernels.
// RK4V4E8 extends the existing INT8 path with nibble packing, Hadamard rotation and E8-lattice
// projection while preserving the same FP16 per-group scale planes.
...
// Orthogonal H64 rotation over two 32-lane halves. This is deliberately implemented with
// shuffle instructions available on Turing SM75; no Ada-only redux or cp.async requirement.
