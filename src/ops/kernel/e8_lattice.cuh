...
// Ported from UDPSendToFailed/ninfer-4090; kept free of Ada-only instructions for SM75.
...
#pragma unroll
    for (int i = 0; i < 8; ++i) { out[i] = (dist_d8 <= dist_coset1) ? d8[i] : coset1[i]; }
}

// Warp-cooperative 8D projection across one 8-lane subgroup. All instructions are supported on
// Turing SM75; do not replace these reductions with Ada redux.sync in this port.
__device__ __forceinline__ float e8_project_8d_warp_single(float x, int lane,
                                                            unsigned sub_mask) {
...
#pragma unroll
    for (int offset = 1; offset < 8; offset <<= 1) {
        const float other_err = __shfl_xor_sync(sub_mask, max_err, offset);
        const int other_lane = __shfl_xor_sync(sub_mask, worst_lane, offset);
    ...
    const float xs = x - 0.5f;
    const float f_s = rintf(xs);
...
#pragma unroll
    for (int offset = 1; offset < 8; offset <<= 1) {
        const float other_err = __shfl_xor_sync(sub_mask, max_err_s, offset);
        const int other_lane = __shfl_xor_sync(sub_mask, worst_lane_s, offset);
    ...
    const float diff0 = x - d8;
    const float diff1 = x - coset1;
